    ...
    <el-card>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="商品图片">
          ...
          <div class="upload-tip">最多上传5张图片（选填）</div>
        </el-form-item>
...
const loadItem = async () => {
  try {
    ...
    if (res.code === 200) {
      form.title = res.data.item.title
      form.description = res.data.item.description
      form.price = res.data.item.price
      form.images = res.data.images?.map(img => img.imageUrl) || []
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    const res = await itemAPI.updateItem(route.params.id, {
      title: form.title,
